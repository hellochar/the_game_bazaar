...
UIRenderer.prototype.renderConnection = function(startNode, endNode) {
    this.ctx.lineWidth = 5;
    this.ctx.globalAlpha = 1;
    this.ctx.strokeStyle = "green";
    this.ctx.beginPath();
    this.ctx.moveTo(startNode.pos.x, startNode.pos.y);
    this.ctx.lineTo(endNode.pos.x, endNode.pos.y);
    this.ctx.stroke();
};
...
UIRenderer.prototype.renderGS = function(gamestate) {
    ...
    gamestate.obstacles.nodes.forEach(function (startNode) {
        startNode.connections.forEach(function (endNode) {
            this.renderConnection(startNode, endNode);
        }.bind(this));
    }.bind(this));
...
UIRenderer.prototype.setSelection = function(selectedUnits) {
