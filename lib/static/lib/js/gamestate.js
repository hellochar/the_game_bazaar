...
GameState.prototype.addUnit = function(player, position, speed, size, cooldown) {
    var unit = new Unit(position, speed, size, cooldown, player);
...
//                                UNIT
//=============================================================================
function Unit(init_pos, speed, size, cooldown, player) {
    ...
    this.size = size || 15;
    // This unit is initially unselected
    this.selected = false;
...
Unit.prototype.update = function(t, destination) {
    ...
    var node_list = getPath(start, destination, this.player.gamestate.obstacles, this.size);
    if (node_list === null) {
        return false;
    }
...
Unit.prototype.shootBullet = function(startTime) {
    if (startTime < this.lastShot + this.cooldown) {
        return;
    }
    this.lastShot = startTime;
...
Unit.fromJSON = function(unit_data, player) {
    ...
    var cooldown = unit_data['cooldown'];
    return new Unit(vect, speed, size, cooldown, player);
};
...
Unit.prototype.evaluate = function(t) {
    ...
    var livingBullets = this.bullets.filter(function(bullet) {
        return (bullet.endTime >= t && bullet.startTime <= t);
    }.bind(t));
    var evaluatedBullets = livingBullets.map(function(bullet){
