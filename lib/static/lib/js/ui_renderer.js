...
UIRenderer.prototype.renderMap = function(gamestate, player_id) {
    ...
    this.ctx.beginPath();
    this.ctx.rect(0, 0, this.minimapSize, this.minimapSize);
    this.ctx.fill();
    this.ctx.lineWidth = 1;
    this.ctx.stroke();
...
// Feed in 2D coordinates of the viewport edges (any quadrilateral as long as they are in order).
// These coordinates must be normalized. That is, they hold the percentage across the map in
// either direction rather than the absolute coordinates.
