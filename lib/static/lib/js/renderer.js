...
Renderer.prototype.renderUnitSelectedHighlight = function(unit, nowtime, starttime) {
    var pos = unit.pos(nowtime - starttime);
    this.ctx.lineWidth = 5;
    this.ctx.globalAlpha = 1;
    this.drawCircle(pos.x, pos.y, unit.size - Math.floor(this.ctx.lineWidth / 2), '#00CC00');
    this.ctx.globalAlpha = 0.4;
};
...
Renderer.prototype.refreshAll = function(game) {
    ...
    if (game.conn_state == game.GAME_STATES.CONNECTED) {
        ...
        players[game.player_id].selectedUnits.forEach(function(unit) {
            this.renderUnitSelectedHighlight(unit, nowtime, game.client_start_time);
        }.bind(this));
