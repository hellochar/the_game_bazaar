...
function GSRenderer() {
    ...
    // The view angle is calculated by atan2(cameraHeight, cameraBack)
    this.cameraBack = 250;
    ...
    objLoader.load( '/static/game/assets/Biplane.obj', function(object) {
        this.bp_geometry = this.processObject3d(object);
    }.bind(this));
...
GSRenderer.prototype.resize = function() {
    // Change the width and height of the renderer (this function will take in two arguments later)
    this.width = window.innerWidth;
    this.height = window.innerHeight;
...
GSRenderer.prototype.project = function(pt) {
    var vector = new THREE.Vector3( ( pt.x / this.width ) * 2 - 1, - ( pt.y / this.height ) * 2 + 1, 1);
    this.projector.unprojectVector( vector, this.camera );
    var dir = vector.sub( this.camera.position ).normalize();
    if (debug) {
        console.log("Now camera position is: " + this.camera.position.x + ", " + this.camera.position.y + ", " + this.camera.position.z);
        console.log("Dir is: " + dir.x + ", " + dir.y + ", " + dir.z);
    }
...
GSRenderer.prototype.processObject3d = function(object) {
...
GSRenderer.prototype.makeTerrain = function(terrain) {
    ...
    var plane = new THREE.Mesh(
        new THREE.PlaneGeometry( terrain.width, terrain.height, 20, 20 ),
        new THREE.MeshLambertMaterial( { map: this.terrainTexture, side: THREE.DoubleSide } )
    );
    this.setTarget(terrain.width / 2, terrain.height / 2);
    plane.position.set(terrain.width / 2, terrain.height / 2, 0);
...
GSRenderer.prototype.makeObstacle = function(startNode, endNode) {
    ...
    geometry.vertices.push(endNode.pos.clone().add(new THREE.Vector3(0, 0, 20)));
    geometry.vertices.push(startNode.pos.clone().add(new THREE.Vector3(0, 0, 20)));
...
GSRenderer.prototype.makeUnit = function(color) {
    var geometry = this.bp_geometry || this.cone_geometry;
...
GSRenderer.prototype.makeBullet = function() {
    // By default the mesh is a cone of size 7
...
GSRenderer.prototype.updatePosAndFacing = function(mesh, obj) {
    ...
    // set the z coordinate of the object to above 20 to make
    // it visible above the plane.
    mesh.position.set(obj.pos.x, obj.pos.y, 20);
