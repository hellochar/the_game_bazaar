...
function GSRenderer() {
    ...
    // We will need this when we work with textures as well
    ...
    // } );
    // imgLoader.load( 'porco.jpg' );

    var objLoader = new THREE.OBJLoader();

    // When it finally finishes loading the event handler will be called
    objLoader.load( '/static/game/assets/Biplane2.obj', function( object ) {
    ...
    objLoader.load( '/static/game/assets/stunt.obj', function( object ) {
        console.log("Done loading obj!");
        object.traverse( function ( child ) {
            if ( child instanceof THREE.Mesh ) {
                if (!this.stnt_geometry) {
                    this.stnt_geometry = child.geometry;
                } else {
                    // In case there are multiple meshes that make up this object
                    // Merge their geometries to speed up rendering time
                    THREE.GeometryUtils.merge(this.stnt_geometry, child);
                }
            }
        }.bind(this) );
    }.bind(this) );
...
GSRenderer.prototype.makeUnit = function(color) {
    ...
    if (this.bp_geometry && this.stnt_geometry) {
        if (color === "rgb(255, 0, 0)") {
            geometry = this.stnt_geometry;
        } else {
            geometry = this.bp_geometry;
        }
    }

    mesh = new THREE.Mesh(geometry, this.color_material[color]);
    mesh.overdraw = true;
    mesh.castShadow = true;
    mesh.receiveShadow = false;
